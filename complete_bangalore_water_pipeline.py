"""
Complete Automated Pipeline for Bangalore Water Analysis (2020-2023)
...
def main():
    print("=" * 80)
    print("💧 COMPLETE AUTOMATED PIPELINE: BANGALORE WATER ANALYSIS (2020-2023)")
    print("=" * 80)
    print(f"🕐 Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("\n📍 Location: Bangalore, Karnataka, India")
    print("📅 Period: 2020 - 2023 (4 years)")
    print("🌊 Analyzing: Water body changes and patterns")
    print("=" * 80)

    # Configuration
    output_dir = Path("analysis_output/bangalore_water_2020_2023_complete")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Bangalore bounding box
    bbox = [77.4, 12.8, 77.8, 13.1]  # Bangalore area
    years = [2020, 2021, 2022, 2023]
    months = [(6, 9)]  # Monsoon season for better water detection
    ...
    print("\n" + "=" * 80)
    print("PHASE 2: Downloading Satellite Data (2020-2023)")
    print("=" * 80)
    ...
    for year in years:
        ...
        if len(existing_images) >= 5:
            ...
            try:
                # Search for images during monsoon season
    ...
    # PHASE 5: Analyze water body patterns AND generate visualizations
    print("\n" + "=" * 80)
    print("PHASE 5: Water Body Pattern Analysis & Visualization Generation")
    print("=" * 80)
    ...
    # Calculate water coverage for each year
    print(f"\n{'─' * 60}")
    print(f"💧 Calculating Water Coverage Area...")
    print(f"{'─' * 60}")
    
    # Bangalore area: approximately 709 km²
    ...
    water_coverage_by_year = {}
    
    for year in years_list:
        features = all_features_by_year[year]
        
        # Use clustering to identify water bodies
        # Water typically has distinct spectral signatures (low in red, high in blue/green)
        # We'll use the feature magnitude to identify water vs non-water
        
        # Method: Calculate feature statistics and use thresholding
        # Lower feature values in MAE typically indicate water (simpler texture)
        # Higher values indicate urban/vegetation (complex texture)
        
        feature_means = features.mean(axis=1)
        
        # Use adaptive thresholding (bottom 30% likely water bodies)
        # This is based on the fact that water has simpler patterns
        threshold = np.percentile(feature_means, 30)
        
        water_tiles = (feature_means < threshold).sum()
        total_tiles = len(features)
        
        # Calculate coverage
        water_area_km2 = water_tiles * tile_area_km2
        coverage_percentage = (water_tiles / total_tiles) * 100
        
        water_coverage_by_year[year] = {
        ...
        print(f"\n  📅 Year {year}:")
        print(f"     💧 Water Tiles: {water_tiles}/{total_tiles}")
        print(f"     📏 Water Area: {water_area_km2:.2f} km²")
        print(f"     📊 Coverage: {coverage_percentage:.1f}%")
    
    # Save water coverage data
    coverage_file = output_dir / "water_coverage_data.npy"
    np.save(coverage_file, water_coverage_by_year)
    print(f"\n  💾 Water coverage data saved to: {coverage_file}")
    ...
    for year in years_list:
        ...
        # Clustering to identify different land/water types
        n_clusters = 5
        ...
        axes[0, 0].set_title(f'{year} - Water Body Patterns\n(PCA Visualization)', 
        ...
        axes[1, 0].set_title(f'{year} - Water Indicator Distribution', 
                            fontsize=14, fontweight='bold')
        ...
        # Get water coverage for this year
        water_info = water_coverage_by_year[year]
        ...
        stats = {
            ...
            'Water Body Area': f"{water_info['water_area_km2']:.2f} km²",
            'Water Coverage': f"{water_info['coverage_percentage']:.1f}%",
            'Water Tiles': f"{water_info['water_tiles']}/{water_info['total_tiles']}",
    ...
    if len(years_list) >= 2:
        ...
        axes[0, 0].set_title(f'Before vs After: Water Body Distribution\n{first_year} vs {last_year}', fontsize=14, fontweight='bold')
        ...
        axes[1, 0].set_title(f'PCA: Water Body Patterns ({first_year} vs {last_year})', fontsize=14, fontweight='bold')
        ...
        change_percent = ((values_last[0] - values_first[0]) / abs(values_first[0])) * 100
        axes[1, 1].axis('off')
...
Year {first_year} (Before):
  Mean: {values_first[0]:.4f}
  Std: {values_first[1]:.4f}
  Samples: {len(features_first):,}

Year {last_year} (After):
  Mean: {values_last[0]:.4f}
  Std: {values_last[1]:.4f}
  Samples: {len(features_last):,}
...
Interpretation:
{'⚠️ Water bodies contracting' if change_percent < 0 else '✓ Water bodies stable/expanding'}
"""
    ...
    if len(years_list) >= 2:
        ...
        means = [all_features_by_year[year].mean() for year in years_list]
        stds = [all_features_by_year[year].std() for year in years_list]
        
        axes[0, 0].plot(years_list, means, 'b-', linewidth=3, marker='o', markersize=10, label='Mean')
        axes[0, 0].fill_between(years_list, 
                                [m - s for m, s in zip(means, stds)],
                                [m + s for m, s in zip(means, stds)],
                                alpha=0.3, color='blue', label='±1 Std')
        axes[0, 0].set_title(f'Historical Water Body Trend ({years_list[0]}-{years_list[-1]})', fontsize=14, fontweight='bold')
        axes[0, 0].set_xlabel('Year', fontsize=12)
        axes[0, 0].set_ylabel('Mean Feature Value', fontsize=12)
        axes[0, 0].legend(fontsize=11)
        axes[0, 0].grid(True, alpha=0.3)
        
        for year, mean in zip(years_list, means):
            axes[0, 0].annotate(f'{mean:.3f}', (year, mean), textcoords="offset points",
                               xytext=(0, 15), ha='center', fontsize=9, fontweight='bold')
        
        # Plot 2: Variance trend
        variances = [all_features_by_year[year].var() for year in years_list]
        axes[0, 1].bar(years_list, variances, color='steelblue', edgecolor='black', alpha=0.8)
        ...
        axes[1, 0].set_title('Year-over-Year Water Body Change', fontsize=14, fontweight='bold')
        axes[1, 0].set_ylabel('Change (%)', fontsize=12)
        ...
        # Plot 4: Summary with water coverage data
        axes[1, 1].axis('off')
        
        total_water_area_first = water_coverage_by_year[years_list[0]]['water_area_km2']
        total_water_area_last = water_coverage_by_year[years_list[-1]]['water_area_km2']
        area_change = total_water_area_last - total_water_area_first
        coverage_first = water_coverage_by_year[years_list[0]]['coverage_percentage']
        coverage_last = water_coverage_by_year[years_list[-1]]['coverage_percentage']
        
        trend_summary = f"""
WATER BODY TREND ANALYSIS
━━━━━━━━━━━━━━━━━━━━━━━━━

Period: {years_list[0]} - {years_list[-1]} ({len(years_list)} years)

Water Coverage Statistics:
{chr(10).join([f'  {year}: {water_coverage_by_year[year]["water_area_km2"]:.2f} km² ({water_coverage_by_year[year]["coverage_percentage"]:.1f}%)' for year in years_list])}

Trend Analysis:
  Start ({years_list[0]}): {total_water_area_first:.2f} km² ({coverage_first:.1f}%)
  End ({years_list[-1]}): {total_water_area_last:.2f} km² ({coverage_last:.1f}%)
...
Assessment:
{'⚠️ Decreasing trend - Water bodies may be shrinking' if area_change < 0 else '✓ Stable/Increasing trend - Water bodies maintained'}
"""
    ...
    print("=" * 80)
    print(f"📍 Location: Bangalore, Karnataka, India")
    print(f"📅 Period: 2020 - 2023")
    ...
    print(f"  • {tile_size}×{tile_size} tiles created")
    print(f"  • Features extracted using MAE model")
    print(f"  • Year-by-year pattern analysis completed")
